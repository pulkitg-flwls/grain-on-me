...
def process_video(input_path, output_path, preset=GrainPreset.KODAK_5248, intensity_scale=1.0):
    """
    ...
    try:
        with tqdm(total=frame_count, desc=f"Processing {os.path.basename(input_path)}", unit="frame") as pbar:
            while True:
                ...
                grained_frame = apply_film_grain(frame, preset=preset, intensity_scale=intensity_scale, blend_mode=BlendMode.ADD)
...
if __name__ == "__main__":
    ...
    for video in files[1:]:
        ...
        process_video(input_path, hard_path, preset=GrainPreset.KODAK_VISION3, intensity_scale=0.4)
        process_video(input_path, easy_path, preset=GrainPreset.KODAK_VISION3, intensity_scale=0.1)